...
def load_and_cache_defect_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    if os.path.exists(cache_fn):
        ...
        domian_labels = torch.tensor([f.domain_label for f in features], dtype=torch.long)
        data = TensorDataset(all_source_ids, all_labels,domian_labels)

        if args.local_rank in [-1, 0] and args.data_num == -1:
            torch.save(data, cache_fn)
    return examples, data

def load_and_cache_defect_train_data(args, filename, pool, tokenizer, split_tag,is_sample=False):
    cache_fn = os.path.join(args.cache_path, split_tag)
    examples = read_examples(filename, args.data_num, args.task)
    calc_stats(examples, tokenizer, is_tokenize=True)
    result,d = split_by_query(examples)
    result2 = list()

    num = len(examples)
    if os.path.exists(cache_fn):
        logger.info("Load cache data from %s", cache_fn)
        result2 = torch.load(cache_fn)
    else:
        if is_sample:
            logger.info("Sample 10 percent of data from %s", filename)
        elif args.data_num == -1:
        ...
        if args.local_rank in [-1, 0] and args.data_num == -1:
            torch.save(result2, cache_fn)

    return result,result2,d,num
